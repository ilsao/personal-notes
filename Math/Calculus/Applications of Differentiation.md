...
因為 $f'(c)$ 存在，所以 $f'(c)=\lim_{ h \to 0 } \frac{f(c+h)-f(c)}{h}=\lim_{ h \to 0^{+} } \frac{f(c+h)-f(c)}{h}\leq 0$。
如果 $h<0$，我們有 $\frac{f(c+h)-f(c)}{h}\geq 0$。
對此式取左極限 $\lim_{ h \to 0^{-} } \frac{f(c+h)-f(c)}{h} \geq 0$。
...
- 尋找全局極值時，除了比對關鍵點外，還需要**確保該點在給定的區間內**。
