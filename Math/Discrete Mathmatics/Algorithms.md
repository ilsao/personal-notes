...
考慮：為 $m$ 元換幣，使用最少硬幣表達 $m$ 元。以下是可使用的單位 $\{ 1,2,5,10,20,50,100,200 \}$。
...
![[Pasted image 20260409210917.png|663]]
...
最差情況：對反序鏈錶 (插入時省去搬運，又或者數組搬運次數與比較次數相同)，
需要 $0+1+2+\dots+(n-1)=\frac{1}{2}n^{2}-\frac{1}{2}n$ 次比較，時間複雜度為 $O(n^{2})$。
