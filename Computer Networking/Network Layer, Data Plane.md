...
再次考慮 $R_{\text{switch}}$ 為 $R_{\text{line}}$ 的 $N$ 倍的情況，此時輸出端口每一秒會接收到 $N^{2}$ 個分組 (假設 $N$ 個輸入端口都往同一個輸出端口輸出封包)，而輸出端口的轉發速率才 $N\text{ pkts/sec}$，勢必會發生排隊。
...
通常，路由器地址由管理員手動配置。但是，(子網內的) 主機地址通常由**動態主機配置協議(Dynamic Host Configuration Protocol, DHCP)** 分配。
