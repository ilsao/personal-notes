...
我們通過將帶有小孔的空腔加熱到所需溫度，並在小孔外部安裝測量設備 (如光譜儀)，可以獲補小孔漏出的輻射。由於小孔非常小，這些輻射幾乎全都來自空腔內部的表面。這樣就可以測得黑體輻射的輻射頻率。

## Character of the Spectrum of a Blackbody

黑體發出的光譜有如下特徵：
1. 若將輻射繪製成頻率與強度的圖表，我們可以看到較寬且連續的圖形。注意到 $y$ 軸的單位是 $I(\nu)$，即每單位時間單位面積下，固定頻率所發出的能量。
2. 將 $I(\nu)$ 對 $\nu$ 積分，其結果不受頻率影響而與溫度四次方正比。此事實被稱為 Stefan-Boltzmann law。其中 $\boxed{\sigma=5.67\times 10^{-8}\text{ W/m}^{2}\text{-K}^{4}}$ 
$$
...
> [!note]
> 為了解釋黑體輻射，普朗克將**原子簡諧運動體收放的能量**量子化。
...
**單色光(monochromatic light)** (單波長) 透過**石英(quartz)** 窗照射在 $C$ 上。因為陽極 $A$ 相對於 $C$ 具有負電位 $V$，被激發出的電子會遭遇**減速電壓(retarding voltage)** $V$。為了使**光電子(photoelectron)** 到達陽極，它們必須以大於位能 $|e|V$ 的動能 $E_{k}$ 被發出。

> [!note]
> 因為 A 具有負電位，電子亦帶負電，所以會有減速電壓。
...
事實上，圖 (c) 也沒法被解釋。古典電磁學認為電磁波能量依賴於強度而非頻率。這無法解釋 $V_{0}$ 為何依賴於頻率，也無法解釋為何存在 $V_{c}$ 使無論光強度多大都無法產生光電流。

最後，更無法解釋為何光電流瞬間產生。

考慮面積為 $1\text{ m}^{2}$ 的金屬板，照射強度為 $10^{-10}\text{ W/m}^{2}$ 的光，假設某一原子上的所有能量都被某一電子吸收。而金屬原子間距為 $d\approx2\times10^{-10}\text{ m}$，束縛能為 $1\text{ eV}$。
...
愛因斯坦說，頻率為 $\nu$ 的電磁波能量不集中在波前，而是集中在稱為**光子(photon)** 的小能量束 (具有粒子性) 上。其中每個光子的能量為：
...
因為有 $\boxed{E_{k\text{,max}}=eV_{0}}$，所以上式可寫成：
...
Sol:
完美的電磁波吸收體與放射體，其放出的輻射只與溫度有關而不受組成元素影響。
